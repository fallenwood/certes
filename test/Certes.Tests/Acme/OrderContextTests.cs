...
public class OrderContextTests
{
    ...
    public async Task CanLoadAuthorizations()
    {
        ...
        httpClientMock
            .Setup(m => m.Post<Order>(location, It.IsAny<JwsPayload>(), It.IsAny<JsonTypeInfo>(), It.IsAny<JsonTypeInfo<Order>>()))
            .Callback((Uri _, object o, JsonTypeInfo requestJsonTypeInfo, JsonTypeInfo<Order> responseJsonTypeInfo) =>
            {
                var p = (JwsPayload)o;

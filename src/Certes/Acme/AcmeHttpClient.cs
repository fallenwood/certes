...
public class AcmeHttpClient : IAcmeHttpClient
{
    ...
    /// <returns></returns>
    public async Task<AcmeHttpResponse<T>> Get<T>(Uri uri, JsonTypeInfo<T> jsonTypeInfo)
    {
        ...
        return await ProcessResponse<T>(response, uri, jsonTypeInfo);
    }
    ...
    /// <param name="requestJsonType"></param>
    /// <param name="responseJsonType"></param>
    /// <returns></returns>
    public async Task<AcmeHttpResponse<TResponse>> Post<TRequest, TResponse>(
        Uri uri,
        TRequest payload,
        ...
        JsonTypeInfo<TResponse> responseJsonType)
    {
        ...
        var payloadJson = JsonSerializer.Serialize(payload, requestJsonType);
        ...
        AddUserAgentHeader(msg);
        using var response = await Http.SendAsync(msg);
        return await ProcessResponse<TResponse>(response, uri, responseJsonType);
    }
    ...
    private async Task<AcmeHttpResponse<T>> ProcessResponse<T>(
        ...
        JsonTypeInfo<T> jsonTypeInfo)
    {
        ...
        if (response.IsSuccessStatusCode)
        {
            if (IsJsonMedia(response.Content?.Headers.ContentType?.MediaType))
            {
                ...
                resource = JsonSerializer.Deserialize(json, jsonTypeInfo);
            }
        ...
        else
        {
            ...
            {
                var json = await response.Content.ReadAsStringAsync();
                error = JsonSerializer.Deserialize(json, AcmeJsonContext.Default.AcmeError);
            }
            else
            {
    ...
    {
        newNonceUri = newNonceUri ?? (await Get(directoryUri, AcmeJsonContext.Default.Directory)).Resource.NewNonce;

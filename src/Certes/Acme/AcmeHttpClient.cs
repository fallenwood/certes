...
public class AcmeHttpClient : IAcmeHttpClient
{
    ...
    /// <inheritdoc />
    public async Task<AcmeHttpResponse<T>> Get<T>(Uri uri, JsonTypeInfo<T> responseJsonTypeInfo)
    {
        ...
        return await ProcessResponse(response, uri, responseJsonTypeInfo);
    }

    /// <inheritdoc />
    public async Task<AcmeHttpResponse<T>> Post<T>(Uri uri, object payload, JsonTypeInfo requestJsonTypeInfo, JsonTypeInfo<T> responseJsonTypeInfo)
    {
        var payloadJson = JsonSerializer.Serialize(payload, requestJsonTypeInfo);
        ...
        AddUserAgentHeader(msg);
        using var response = await Http.SendAsync(msg);
        return await ProcessResponse<T>(response, uri, responseJsonTypeInfo);
    }
    ...
    private async Task<AcmeHttpResponse<T>> ProcessResponse<T>(HttpResponseMessage response, Uri requestedUri, JsonTypeInfo<T> responseJsonTypeInfo)
    {
        ...
        if (response.IsSuccessStatusCode)
        {
            if (IsJsonMedia(response.Content?.Headers.ContentType?.MediaType))
            {
                ...
                resource = JsonSerializer.Deserialize(json, responseJsonTypeInfo);
            }
        ...
        else
        {
            ...
            {
                var json = await response.Content.ReadAsStringAsync();
                error = JsonSerializer.Deserialize<AcmeError>(json, AcmeJsonSerializerContext.Indented.AcmeError);
            }
            else
            {
    ...
    {
        newNonceUri = newNonceUri ?? (await Get<Directory>(directoryUri, AcmeJsonSerializerContext.Unindented.Directory)).Resource.NewNonce;

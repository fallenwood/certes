...
internal class OrderListContext : EntityContext<OrderList>, IOrderListContext
{
    ...
    public OrderListContext(IAcmeContext context, Uri location)
        : base(context, location, AcmeJsonSerializerContext.Unindented.OrderList)
    {
    }
    ...
    public async Task<IEnumerable<IOrderContext>> Orders()
    {
        ...
        while (next != null)
        {
            var resp = await Context.HttpClient.Get<OrderList>(next, AcmeJsonSerializerContext.Unindented.OrderList);
